'''Terrain-specific helpers for distances, azimuth binning, and sky masks.

General coordinate transforms (``rot_m``, ``xyz2thphi``, ...) live in
``healjax.coord``; import them from there rather than redefining them here.
'''
import numpy as np
...
def rot_m(ang, vec):
    """Return the right-handed rotation matrix about ``vec`` by ``ang``.

    Newer healjax releases provide this helper. Keep the NumPy equivalent
    here so image-pose tools also run with the declared older release.
    """
    c = np.cos(ang)
    s = np.sin(ang)
    C = 1 - c
